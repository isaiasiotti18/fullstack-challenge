...
import { EXCHANGE_NAME, WALLETS_QUEUE } from "./events";

const MAX_RETRIES = 3;
const RETRY_DELAY_MS = 5_000;
const ROUTING_KEYS = ["bet.placed", "round.ended"];
...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connect(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        await this.channel.assertQueue(WALLETS_QUEUE, { durable: true });

        for (const key of ROUTING_KEYS) {
          await this.channel.bindQueue(WALLETS_QUEUE, EXCHANGE_NAME, key);
        }
        ...
        this.logger.log(
          `Connected to RabbitMQ. Queue "${WALLETS_QUEUE}" bound to exchange "${EXCHANGE_NAME}" with keys: [${ROUTING_KEYS.join(", ")}]`,
        );
        return;
      } catch (error) {
  ...
  async consume(callback: (routingKey: string, message: unknown) => Promise<void>): Promise<void> {
    ...
    this.logger.log(`Consuming messages from queue "${WALLETS_QUEUE}"`);
  }
